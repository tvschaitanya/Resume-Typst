...
# -- preflight --
# Verify required tool and source files exist
...
# Parse command-line options (-c to clear, -w for watch mode)
...
# -- clear --
# Remove exports directory if clear flag is true
if $CLEAR; then
  rm -rf exports
...
# -- parse metadata --
# Extract name from contact.typ and role from data.typ
...
# Restrict to safe filename characters and generate initials
...
# -- collision check --
# Append numerical suffix if the target file already exists
...
# -- compile --
# Execute typst command and capture output in a temporary log
echo "$CMD → $OUT"
TMPLOG=$(mktemp)
...
# Filter out non-essential warnings from the typst output
